...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            # Left-Arc: add dependency from the second word on the stack to the first
            governor = self.stack[-1]
            dependent = self.stack.pop(-2)
            self.dependencies.append((governor, dependent))
        elif transition == "RA":
            # Right-Arc: add dependency from the first word on the stack to the second
            governor = self.stack[-2]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        unfinished_parses = [pp for pp in unfinished_parses if not pp.finished()]
