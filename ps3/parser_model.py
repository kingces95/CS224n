...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        num_words, embedding_size = embeddings.shape

        assert embedding_size == self.embed_size
        assert n_features == self.n_features
        assert hidden_size == self.hidden_size

        embed_to_hidden_fan_in = n_features * embedding_size
        embed_to_hidden_fan_out = hidden_size
        embed_to_hidden_shape = (embed_to_hidden_fan_in, embed_to_hidden_fan_out) # fan-out x fan-in
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(embed_to_hidden_shape)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(hidden_size)))

        self.dropout = nn.Dropout(p = dropout_prob)

        hidden_to_logits_fan_in = embed_to_hidden_fan_out
        hidden_to_logits_fan_out = n_classes
        hidden_to_logits_shape = (hidden_to_logits_fan_in, hidden_to_logits_fan_out)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(hidden_to_logits_shape)))
    ...
    def embedding_lookup(self, w):
        ...
        num_words, embedding_size = embeddings.shape
        assert embeddings.shape == (num_words, embedding_size)

        batch_size, n_features = w.shape
        assert n_features == self.n_features

        indices = torch.flatten(w)
        assert indices.shape == (batch_size * n_features,)

        assert embeddings.shape == (num_words, embedding_size)
        x = torch.index_select(embeddings, 0, indices).view(batch_size, -1)
        assert x.shape == (batch_size, n_features * embedding_size)
    ...
    def forward(self, w):
        ...
        assert w.shape == (batch_size, n_features)
        embeddings = self.embedding_lookup(w)
        assert embeddings.shape == (batch_size, n_features * self.embed_size)

        assert embeddings.shape == (batch_size, n_features * self.embed_size)
        assert self.embed_to_hidden_weight.shape == (n_features * self.embed_size, self.hidden_size)
        linear = torch.matmul(embeddings, self.embed_to_hidden_weight)
        assert linear.shape == (batch_size, self.hidden_size)

        assert linear.shape == (batch_size, self.hidden_size)
        assert self.embed_to_hidden_bias.shape == (self.hidden_size,)
        linear_biased = linear + self.embed_to_hidden_bias
        assert linear_biased.shape == (batch_size, self.hidden_size)

        assert linear_biased.shape == (batch_size, self.hidden_size)
        hidden = F.relu(linear_biased)
        assert hidden.shape == (batch_size, self.hidden_size)

        assert hidden.shape == (batch_size, self.hidden_size)
        dropout = self.dropout(hidden)
        assert dropout.shape == (batch_size, self.hidden_size)

        assert dropout.shape == (batch_size, self.hidden_size)
        assert self.hidden_to_logits_weight.shape == (self.hidden_size, self.n_classes)
        assert self.hidden_to_logits_bias.shape == (self.n_classes,)
