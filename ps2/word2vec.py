...
def one_hot_at_index(size, i):
    # Write a python statement to create a one-hot vector at index i.
    vec = np.zeros(size)
    vec[i] = 1
    return vec

def is_distribution(vec):
    return np.isclose(vec.sum(), 1, rtol=1e-5, atol=1e-8)
...
def naiveSoftmaxLossAndGradient(
    ...
    # > Write a python function naive_softmax_gradient that accepts 
    # center_word_vec, outside_word_index, and outside_vectors and 
    # returns gradient_center_vec, gradient_outside_vec assuming a 
    # softmax function is available.

    # This is impossible. ChatGTP, after first implementing it wrong, learned
    # after some prompting the correct implementation and now generates it 
    # correctly. Simply impossible. I don't believe it. I refuse to believe it.
    # Yet it moves... :(

    # > Who said, "yet it moves?"
    ...
    y = one_hot_at_index(n, o)
    assert n == len(y)
    assert is_distribution(y)

    y_hat = softmax(U @ v_c)
    assert n == len(y_hat)
    assert is_distribution(y_hat)

    loss = - np.log(y_hat[o])

    gradCenterVec = U.T @ (y_hat - y)
    assert d == len(gradCenterVec)

    gradOutsideVecs = np.outer(y_hat - y, v_c)
    assert U.shape == gradOutsideVecs.shape
...
def negSamplingLossAndGradient(
    ...
    U_hat = signs * U[indices]
    assert U_hat.shape == (K + 1, d)

    score = sigmoid(U_hat @ v_c)
    assert score.shape == (K + 1, )

    loss = - np.log(score).sum()

    # reshape allows for broadcasting
    neg_score = (score - 1).reshape(K + 1, 1)

    gradCenterVec = (neg_score * U_hat).sum(axis = 0)
    assert gradCenterVec.shape == v_c.shape

    du_o_dw_s = neg_score * v_c * signs
    assert U_hat.shape == du_o_dw_s.shape
    assert du_o_dw_s.shape[0] == K + 1

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(K + 1):
        gradOutsideVecs[indices[i]] += du_o_dw_s[i]
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    m = windowSize
    w = outsideWords
    assert len(w) > 0 and len(w) <= 2 * m
    V = centerWordVectors
    U = outsideVectors
    f = word2vecLossAndGradient
    ...
    for o in w:
        ...
        loss += d_loss
        gradCenterVecs[word2Ind[c]] += dv_c
